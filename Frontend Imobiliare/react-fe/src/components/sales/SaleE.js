...
export const SaleE = ({
    ...
    function deleteHandler() {
        const fetchSale = async () => {
            await fetch(`http://127.0.0.1:5000/sales/delete/${id}`, {
                method: 'DELETE',
                headers: {
        ...
        fetchSale().then(r => deleteSaleForElement(id));
    }

    function updateHandler() {
        const updateSale = async () => {
            await fetch('http://127.0.0.1:5000/sales/update', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
        ...
        updateSale().then(r => updateElement());
        setIsModify(false);
    }
